...
import { GroupsInterface, GroupsStateInterface, TenantsInterface } from 'src/store/groups/state';
...
export const create = async ({ profile, avatar }: { profile: GroupsInterface, avatar: File }) => {
  ...
  return axios.post<GroupsInterface>(`${API}/groups`, profile)
    .then(({ data }) => data);
};

export const addTenanttoGroup = async (id: string, payload: AddTenantInterface) => {
  return axios.post<GroupsStateInterface>(`${API}/groups/${id}/tenants`, payload)
    .then(({ data }) => data);
};
...
export const update = async ({ profile: { id, ...updatedProfile }, avatar }: { profile: GroupsInterface, avatar: File }) => {
  ...
  return axios.put<GroupsStateInterface>(`${API}/groups/${id}`, updatedProfile)
    .then(({ data }) => data);
};
